...
function PayModal({ installment, accounts, onConfirm, onClose }: PayModalProps) {
        ...
        <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white w-full max-w-xs rounded-xl shadow-2xl" onClick={e => e.stopPropagation()}>
                <div className="px-5 py-4 border-b border-gray-100">
                    <h3 className="text-base font-bold text-navy-900">Pagar cuota</h3>
                    <p className="text-xs text-gray-500 mt-0.5 truncate">{installment.description} — cuota {installment.installment_number}/{installment.num_installments}</p>
                </div>
                <div className="px-5 py-4 space-y-3">
                    <div className="bg-navy-50 rounded-lg px-4 py-3 flex justify-between items-center">
                        ...
                        <span className="text-xl font-black text-navy-900">{formatCurrency(installment.amount)}</span>
                    </div>
                    {accounts.length > 0 ? (
                        <div>
...
export function InstallmentsView({ installments, accounts, onPayInstallment }: Props) {
