...
interface Props {
    supplier?: Supplier
    onSave: (data: { name: string; category: string; phone?: string; notes?: string }) => void
    onClose: () => void
    onDelete?: () => void
    deleteError?: string
}

export function SupplierModal({ supplier, onSave, onClose, onDelete, deleteError }: Props) {
    const [name, setName] = useState(supplier?.name || '')
    const [category, setCategory] = useState(supplier?.category || 'otro')
    ...
    const handleSave = () => {
        if (!name.trim()) return
        onSave({ name: name.trim(), category, phone: phone.trim() || undefined, notes: notes.trim() || undefined })
    }
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white w-full max-w-md rounded-xl shadow-2xl" onClick={e => e.stopPropagation()}>
                ...
                <div className="px-6 py-5 space-y-4">
                    ...
                    <div>
                        <label className="text-xs font-bold uppercase text-gray-500 mb-1.5 block tracking-wide">Teléfono</label>
                        <input
                            type="tel"
                            value={phone}
                    ...
                    {deleteError && (
                        <p className="text-xs text-red-600 font-medium bg-red-50 border border-red-100 rounded-lg px-3 py-2">{deleteError}</p>
                    )}
                </div>
                ...
                <div className="px-6 pb-5 flex gap-2">
                    ...
                    <button
                        onClick={handleSave}
                        disabled={!name.trim()}
                        className="flex-1 py-2.5 bg-navy-900 text-white font-bold text-sm uppercase rounded-xl shadow hover:bg-navy-800 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
